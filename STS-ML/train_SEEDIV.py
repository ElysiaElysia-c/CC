...
from getDataSEEDIV import data_SEEDIV
import os
import copy
import config
import time
...
def train(train_iter, test_iter, model, model_best, awl, criterion, lr_list, epochs, acc_test_best, people):
    ...
    for ep in range(epochs): # 20轮
        ...
        # 每一个被试的结束 打印结果
        print('people {} 每个被试的总损失 {}: {}'.format(
            people, ep + 1, total_loss))

        print('>>> 这个被试最好的准确率: {}'.format(acc_test_best))
...
def runs(people):
    ...
    train_data, train_label, test_data, test_label = data_SEEDIV(people)
